...
void tua_timer_every_cancel_cl(void* handle) {
    ...
    // Always cancel immediately (thread-safe), then free on the loop thread to
    // avoid races with in-flight timer callbacks.
    if (h->timer != NULL) {
    ...
    if (tua_loop_post(h->loop, tua_timer_every_free_task, h) != TUA_OK) {
        // Best-effort: avoid freeing here to prevent potential UAF; leak instead.
        return;
    }
}
