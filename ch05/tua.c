...
void add_op(tua_state *t, int pos1, int r2, int pos2, int r3, int pos3) {
...
void sub_op(tua_state *t, int pos1, int r2, int pos2, int r3, int pos3) {
    int left = r2==1?tua_toint(t,pos2):pos2;
    int right = r3==1?tua_toint(t, pos3):pos3;
    tua_setnumber(t, pos1, left - right);
}

void mul_op(tua_state *t,int pos1, int r2, int pos2, int r3, int pos3) {
    int left = r2==1?tua_toint(t,pos2):pos2;
    int right = r3==1?tua_toint(t, pos3):pos3;
    tua_setnumber(t, pos1, left * right);
}

void div_op(tua_state *t, int pos1, int r2, int pos2, int r3, int pos3) {
    int left = r2==1?tua_toint(t,pos2):pos2;
    int right = r3==1?tua_toint(t, pos3):pos3;
    tua_setnumber(t, pos1, left / right);
}

void mod_op(tua_state *t, int pos1, int r2, int pos2, int r3, int pos3) {
...
void call_op(tua_state *t, int op, int pos1, int r2, int pos2, int r3, int pos3) {
    switch (op) {
        case OP_ADD:
            add_op(t, pos1, r2,pos2, r3, pos3);
            break;
        case OP_SUB:
            sub_op(t, pos1, r2,pos2, r3, pos3);
            break;
        case OP_MUL:
            mul_op(t, pos1, r2,pos2, r3, pos3);
            break;
        case OP_DIV:
            div_op(t, pos1, r2,pos2, r3, pos3);
            break;
        case OP_MOD:
            mod_op(t, pos1, r2,pos2, r3, pos3);
            break;
        default:
...
int tua_execute(tua_state *t, tua_instruction* instructions, int num_instructions, int labels[], int *argv) {
    ...
    while (pc < num_instructions) {
        ...
        case OP_MOD:
        {
            ...
            call_op(t, instr.opcode, pos1, r2,r2==1?pos2:v2, r3,r3==1 ? pos3 : v3);
            pc++;
...
int main(int argc, const char *argv[]) {
    ...
    assert_equal(fib(v), run_tua("test2.tvm", (int[]){v}, 1));
