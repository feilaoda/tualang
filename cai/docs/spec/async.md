...
## 2.1 `yield`（精确调度指示符）
语法：
- `yield`（语句；见 `cai/parser.g4`）

约束（冻结）：
- `yield` 只能出现在 `async fn` 或 `async lambda` 中；否则编译错误。

语义（冻结）：
- `yield` 是一个协作式挂起点：当前任务在此让出执行权一次，运行时可调度其他任务运行。
- `yield` 不携带 `Option`/解包语义；与错误守卫 `callExpr ? { ... }` 无关。

---

## 2.2 自动让出（Auto Yield）（Status: Draft）
动机：开发者不写 `yield` 也能避免“长 CPU 循环饿死其他任务”。

规则（Draft，未冻结）：
- 编译器可在 `async fn` 的循环回跳点插入隐式 `yield`，但必须满足：
  - 插入点是语法可见、与平台无关的确定位置（例如循环 back-edge）；
  - 插入频率必须是确定的、可配置的（例如通过 `@autoYield(every=1000)` 这类属性），禁止“基于耗时/动态 profiling”的不确定策略；
  - 默认关闭，需显式开启（避免改变并发可见行为）。
