...
void * pool_falloc(pool_t *pool, size_t size)
{
	unsigned char      *m;
	pool_t  *p;
	...
	if (size <= pool->max)
	{
		...
		{
				p->d.last = m + size;
				debug("return block %ld\n", size);
				return m;
		}
		
		debug("alloc block %ld\n", size);
		return pool_palloc_block(pool, size);
	}
